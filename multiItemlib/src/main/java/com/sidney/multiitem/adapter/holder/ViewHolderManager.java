...
import com.sidney.multiitem.item.ItemData;

import java.util.List;

/**
 * 描 述： * ViewHolder的管理类需要继承此类，实现ViewHolder的创建{@link #onCreateViewHolder}与绑定{@link #onBindViewHolder}
 * 作 者：hxl  2022/1/28 15:03
 * 修改描述：
 * 修 改 人：xxx  2022/1/28 15:03
 * 修改版本：
 */
public abstract class ViewHolderManager<T, V extends BaseViewHolder> {
     ...
     * @param params {@link ViewHolderParams}
     */
    public void onBindViewHolder(@NonNull V holder, @NonNull T t, @NonNull ViewHolderParams params, List<Object> payloads) {
        // TODO 如果以后有需要不直接在item view上设置Click事件，在MultiViewHolder增加itemHandlerView属性即可
        if (isClickable()) {
            holder.itemView.setOnClickListener(params.getClickListener());
            holder.itemView.setOnLongClickListener(params.getLongClickListener());
        ...
        onBindViewHolder(holder, t, payloads);
    }
     ...
     * @param payloads
     */
    public void onBindViewHolder(V holder, T t, List<Object> payloads) {
    }
